...
The raw midpoint chain is finalized by `_finalize_path`. It applies a moving-average smoothing window when enabled, then resamples the curve at `centerline.path_resolution_m` up to `centerline.max_path_length_m`.
...
Finalization is intentionally similar to midpoint planning. `_finalize_path` smooths the raw offset path with a moving average, then resamples it at the configured path resolution and length.
